...
class WbFboSheetRepository:
    ...
    def _box_row(self, row: WbFboSheetBox) -> WbFboSheetBoxRow:
        return WbFboSheetBoxRow(
            ...
            product_barcode=str(row.product_barcode or ""),
    ...
    def _assigned_qty_by_barcode(self, session: Session, job_id: int) -> dict[str, int]:
        rows = session.scalars(
            select(WbFboSheetBox).where(
                WbFboSheetBox.job_id == int(job_id),
                WbFboSheetBox.status == BOX_ASSIGNED,
            )
        ).all()
        out: dict[str, int] = {}
        for row in rows:
    ...
    def _job_row(
        ...
        *,
        ...
        box_rows = [self._box_row(row) for row in boxes]
        sku_by_barcode = {
            str(item.barcode or "").casefold(): item for item in product_rows
        }
        for box in box_rows:
            product = sku_by_barcode.get(str(box.product_barcode or "").casefold())
            if product:
                box.sku = product.sku
                box.product_name = product.name
                box.product_id = product.product_id
        ...
        *,
        ...
        with Session(self.engine) as session:
            ...
            for seq, item in enumerate(boxes, start=1):
                product_barcode = str(item.get("product_barcode") or "")
                qty = int(item.get("qty") or 0)
                status = BOX_ASSIGNED if product_barcode and qty > 0 else BOX_PENDING
                ...
                session.add(
                    WbFboSheetBox(
                        ...
                        product_barcode=product_barcode if status == BOX_ASSIGNED else "",
                        item_qty=qty if status == BOX_ASSIGNED else 0,
                        status=status,
    ...
    def mark_boxes_printed(self, job_id: int, box_ids: list[int]) -> list[WbFboSheetBoxRow]:
        ...
        if not ids:
            raise ValueError("Нет коробов для печати")
        with Session(self.engine) as session:
            ...
            if len(rows) != len(ids):
                raise ValueError("Короб не найден в задании")
            for row in rows:
                if row.status == BOX_ASSIGNED:
                    raise ValueError("Нельзя печатать уже присвоенный короб этой кнопкой")
            ...
            session.commit()
            return [self._box_row(row) for row in rows]
    ...
    def next_unprinted_boxes(self, job_id: int, count: int) -> list[WbFboSheetBoxRow]:
        ...
        with Session(self.engine) as session:
            ...
            if not rows:
                raise ValueError("Нет ещё не печатавшихся коробов")
            return [self._box_row(row) for row in rows]
        ...
        *,
        ...
        with Session(self.engine) as session:
            ...
            if box.status == BOX_ASSIGNED:
                raise ValueError("Этот короб уже присвоен товару")
            product = None
            for row in session.scalars(
                select(WbFboSheetProduct).where(WbFboSheetProduct.job_id == int(job_id))
            ).all():
            ...
            box.product_barcode = str(product.barcode)
            box.item_qty = qty
            ...
            if job.status == JOB_STATUS_OPEN:
                job.status = JOB_STATUS_IN_PROGRESS
            job.updated_at_ts = now
            assigned_after = assigned + qty
            if assigned_after >= int(product.qty_plan or 0):
                remaining_products = session.scalars(
                    select(WbFboSheetProduct).where(WbFboSheetProduct.job_id == int(job_id))
                ).all()
                qty_map = self._assigned_qty_by_barcode(session, job_id)
                qty_map[str(product.barcode).casefold()] = assigned_after
                if all(
