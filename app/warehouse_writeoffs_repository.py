...
class WarehouseWriteoffsRepository:
    ...
    def map_product_quantities_by_ids(
        self, writeoff_ids: list[int]
    ) -> dict[int, list[tuple[int, int]]]:
        ids = sorted({int(x) for x in writeoff_ids if int(x) > 0})
        if not ids:
            return {}
        with Session(self.engine) as session:
            rows = session.scalars(
                select(WarehouseWriteoffItem).where(WarehouseWriteoffItem.writeoff_id.in_(ids))
            ).all()
        out: dict[int, list[tuple[int, int]]] = {}
        for row in rows:
            out.setdefault(int(row.writeoff_id), []).append(
