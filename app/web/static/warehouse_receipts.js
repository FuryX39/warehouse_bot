(function (global) {
  ...
  function loadMeta() {
    return fetchJson("/api/warehouse/receipts/meta").then(function (data) {
  ...
  function renderFilterPanel() {
    return (
      '<div class="wh-crm-filters' + (filterPanelOpen ? "" : " hidden") + '" id="whRcFilters">' +
      ...
      '<button type="button" class="wh-btn wh-btn-primary" id="whRcApplyFilter">Применить</button>' +
      '<button type="button" class="wh-btn" id="whRcResetFilter">Сбросить</button>' +
      "</div></div>"
    );
  }
  ...
  function renderItemsBlock() {
    if (!formItems.length) {
      return '<p class="wh-msg" id="whRcItemsEmpty">Товары не добавлены.</p>';
    }
  ...
  function refreshItemsDom(root) {
    var wrap = root.querySelector("#whRcItems");
    if (!wrap) return;
  ...
  function runProductSearch(root) {
    var name = root.querySelector("#whRcSearchName").value.trim();
    var sku = root.querySelector("#whRcSearchSku").value.trim();
    var code = root.querySelector("#whRcSearchCode").value.trim();
    if (!name && !sku && !code) {
      root.querySelector("#whRcSearchResults").innerHTML = '<p class="wh-msg">Укажите хотя бы один параметр поиска.</p>';
      return;
    }
    var url =
      "/api/warehouse/receipts/products/search?name=" + encodeURIComponent(name) +
      "&sku=" + encodeURIComponent(sku) +
      "&code=" + encodeURIComponent(code);
    fetchJson(url)
      .then(function (data) {
        var list = data.products || [];
        var wrap = root.querySelector("#whRcSearchResults");
        if (!list.length) {
      ...
      .catch(function (err) {
        root.querySelector("#whRcSearchResults").innerHTML =
  ...
  function applyPriceType(root, priceTypeId) {
    ...
    fetchJson("/api/warehouse/receipts/price-by-type", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      ...
      .then(function (data) {
        ...
        var menu = root.querySelector("#whRcPriceMenu");
    ...
    fetchJson("/api/warehouse/receipts" + filtersQuery())
      .then(function (data) {
        ...
        root.innerHTML =
          ...
          '<input type="search" id="whRcQuickSearch" class="wh-crm-search" placeholder="Быстрый поиск…" value="' + esc(listFilters.q || "") + '" />' +
          '<button type="button" class="wh-btn" id="whRcToggleFilter">Фильтр</button>' +
          '<button type="button" class="wh-btn wh-btn-primary" id="whRcCreate">+ Оприходование</button>' +
          "</div>" +
          renderFilterPanel() +
          '<div id="whRcListWrap">' +
          (rows
  ...
  function bindListEvents(root) {
    root.querySelector("#whRcCreate").addEventListener("click", function () {
      editingId = null;
      formItems = [];
      renderForm(null);
    });
    root.querySelector("#whRcToggleFilter").addEventListener("click", function () {
      filterPanelOpen = !filterPanelOpen;
      var fp = root.querySelector("#whRcFilters");
  ...
  function renderForm(receiptId) {
    ...
    root.innerHTML = "<p class=\"wh-msg\">Загрузка…</p>";
    var load = receiptId
      ? fetchJson("/api/warehouse/receipts/" + receiptId).then(function (d) {
          return d.receipt;
        })
      : Promise.resolve({
          title: "",
    ...
    load
      .then(function (r) {
        ...
        root.innerHTML =
          ...
          '<div><label>Название</label><input type="text" id="whRcSearchName" /></div>' +
          '<div><label>Артикул</label><input type="text" id="whRcSearchSku" /></div>' +
          '<div><label>Код</label><input type="text" id="whRcSearchCode" /></div>' +
          '<div class="wh-rc-search-btn-wrap"><button type="button" class="wh-btn" id="whRcSearchBtn">Найти</button></div>' +
          "</div>" +
          '<div id="whRcSearchResults" class="wh-rc-search-results"></div></section>' +
          ...
          '<button type="button" class="wh-btn wh-btn-sm" id="whRcPriceBtn">Расценить ▾</button>' +
          '<div class="wh-rc-price-menu hidden" id="whRcPriceMenu">' + priceTypeMenuHtml() + "</div></div></div>" +
          '<div id="whRcItems" class="wh-rc-items">' + renderItemsBlock() + "</div></section>" +
        ...
        root.querySelector("#whRcSearchBtn").addEventListener("click", function () {
          runProductSearch(root);
        });
        ["whRcSearchName", "whRcSearchSku", "whRcSearchCode"].forEach(function (id) {
          root.querySelector("#" + id).addEventListener("keydown", function (e) {
            if (e.key === "Enter") runProductSearch(root);
          });
        });
        root.querySelector("#whRcPriceBtn").addEventListener("click", function (e) {
          e.stopPropagation();
          root.querySelector("#whRcPriceMenu").classList.toggle("hidden");
        });
        ...
        if (receiptId) {
          root.querySelector("#whRcDelete").addEventListener("click", function () {
            ...
            fetchJson("/api/warehouse/receipts/" + receiptId, { method: "DELETE" })
              ...
              .catch(function (err) {
                var msg = root.querySelector("#whRcFormMsg");
  ...
  function saveReceipt(root, receiptId) {
    var msg = root.querySelector("#whRcFormMsg");
    msg.textContent = "";
    ...
    var req = receiptId
      ? fetchJson("/api/warehouse/receipts/" + receiptId, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        })
      : fetchJson("/api/warehouse/receipts", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
  ...
  function renderReceipts(tab, item) {
