...
class WarehouseReceiptsRepository:
    ...
    def _migrate_bin_id_column(self) -> None:
        ensure_integer_column(self.engine, "warehouse_receipts", "bin_id")
        with Session(self.engine) as session:
            rows = session.scalars(select(WarehouseReceipt)).all()
            for row in rows:
                if row.bin_id:
                    continue
                try:
    ...
    def _migrate_locked_column(self) -> None:
        ensure_integer_column(self.engine, "warehouse_receipts", "locked")
    ...
    def map_product_quantities_by_ids(
        ...
        ids = sorted({int(x) for x in receipt_ids if int(x) > 0})
        if not ids:
            return {}
        with Session(self.engine) as session:
            rows = session.scalars(
                select(WarehouseReceiptItem).where(WarehouseReceiptItem.receipt_id.in_(ids))
            ).all()
        out: dict[int, list[tuple[int, int]]] = {}
        for row in rows:
            out.setdefault(int(row.receipt_id), []).append(
